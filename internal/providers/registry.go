...
func Registry() []Provider {
	return withDerivedMetadata([]Provider{
		...
		{ID: "diffbot", Name: "Diffbot", Category: "extraction", Capabilities: []string{"extract.article"}, AuthType: AuthAPIKey, EnvVar: "DIFFBOT_API_KEY", SetupURL: "https://www.diffbot.com/", FreeTier: "10,000 credits/month", LimitModel: "monthly credits + calls/min", LimitConfidence: "documented", Status: MetadataOnly},
		...
		{ID: "openalex", Name: "OpenAlex", Category: "scholarly", Capabilities: []string{"search.scholar", "resolve.identity", "enrich.citations"}, AuthType: AuthNone, SetupURL: "https://openalex.org/", FreeTier: "$1/day allowance", LimitModel: "daily allowance", LimitConfidence: "documented", Status: CapabilitySupported},
		{ID: "semantic_scholar", Name: "Semantic Scholar Academic Graph API", Category: "scholarly", Capabilities: []string{"search.scholar", "enrich.citations"}, AuthType: AuthAPIKey, EnvVar: "SEMANTIC_SCHOLAR_API_KEY", SetupURL: "https://www.semanticscholar.org/product/api", FreeTier: "Public API, key recommended", LimitModel: "RPS / shared pool", LimitConfidence: "documented", Status: MetadataOnly},
		...
		{ID: "gdelt", Name: "GDELT", Category: "news corpus", Capabilities: []string{"search.news", "archive.lookup"}, AuthType: AuthNone, SetupURL: "https://www.gdeltproject.org/", FreeTier: "Free/open access", LimitModel: "fair use/open corpus", LimitConfidence: "documented", Status: CapabilitySupported},
		...
		{ID: "wordpress", Name: "WordPress REST API", Category: "blogs", Capabilities: []string{"search.platform"}, AuthType: AuthNone, SetupURL: "https://developer.wordpress.org/rest-api/", FreeTier: "Per-site public API", LimitModel: "per-site", LimitConfidence: "documented", Status: MetadataOnly},
		{ID: "wordpress_com", Name: "WordPress.com REST API", Category: "blogs", Capabilities: []string{"search.platform"}, AuthType: AuthOAuth, EnvVar: "WORDPRESS_COM_TOKEN", SetupURL: "https://developer.wordpress.com/docs/api/", FreeTier: "Public endpoints + OAuth", LimitModel: "responsible use", LimitConfidence: "documented", Status: MetadataOnly},
		{ID: "blogger", Name: "Blogger API v3", Category: "blogs", Capabilities: []string{"search.platform"}, AuthType: AuthAPIKey, EnvVar: "BLOGGER_API_KEY", SetupURL: "https://developers.google.com/blogger/docs/3.0/using", FreeTier: "Google API quota", LimitModel: "Google Cloud quotas", LimitConfidence: "documented", Status: MetadataOnly},
...
func setupGroup(p Provider) string {
	...
	if p.ID == "browserless" || p.ID == "scrapingant" || p.ID == "apify" {
